...
import { getZoneRisk, rankZonesForFan, ZoneRisk } from "./src/decision-engine/riskScoring";
...
try {
  if (fs.existsSync(ZONES_PATH)) {
    ...
    console.warn("zones.json not found at standard path, initializing empty state.");
  }
} catch (error) {
...
let activeIncidents: Incident[] = [
  {
    ...
    severity: "warning",
    message: "Temporary ticket reader malfunction causing slow turnstile throughput.",
    status: "active"
  }
];
...
function checkRateLimit(ip: string): boolean {
  ...
  if (!bucket) {
    ...
    bucket.tokens = Math.min(RATE_LIMIT_CAPACITY, bucket.tokens + elapsedSeconds * RATE_LIMIT_REFILL_RATE);
...
function checkOperationsRateLimit(ip: string): boolean {
  ...
  if (!bucket) {
    ...
    const elapsedSeconds = (now - bucket.lastRefill) / 1000;
    bucket.tokens = Math.min(OPS_RATE_LIMIT_CAPACITY, bucket.tokens + elapsedSeconds * OPS_RATE_LIMIT_REFILL_RATE);
    bucket.lastRefill = now;
  }
...
app.post("/api/incidents", (req, res) => {
  ...
  if (!zoneId || !zoneName || !severity || !message) {
    res.status(400).json({ error: "Missing required incident fields (zoneId, zoneName, severity, message)." });
    return;
  }
  ...
  if (severity !== "info" && severity !== "warning" && severity !== "critical") {
    res.status(400).json({ error: "Invalid severity level. Must be 'info', 'warning', or 'critical'." });
    return;
  }
  if (zoneId.length > 30 || zoneName.length > 100 || message.length > 300) {
    res.status(400).json({ error: "Input exceeds maximum allowed length constraints (zoneId: 30, zoneName: 100, message: 300)." });
    return;
  }
...
app.post("/api/incidents/resolve", (req, res) => {
  ...
  if (!id || typeof id !== "string") {
    res.status(400).json({ error: "Missing or invalid incident ID to resolve." });
    return;
  }
  if (id.length > 30) {
...
app.post("/api/simulation/scenario", (req, res) => {
  ...
  const validScenarios = ["normal", "crowd_rush", "gate_b_emergency", "extreme_rain", "egress_surge"];
  if (!validScenarios.includes(scenario)) {
    res.status(400).json({ error: `Invalid scenario. Must be one of: ${validScenarios.join(", ")}` });
    return;
  }
  ...
  if (scenario === "normal") {
    liveZones = liveZones.map(z => {
      if (z.id === "gate-a") { z.currentDensity = 1.2; z.flowPerMinute = 45; }
      if (z.id === "gate-b") { z.currentDensity = 4.8; z.flowPerMinute = 18; }
      if (z.id === "gate-c") { z.currentDensity = 2.1; z.flowPerMinute = 38; }
      if (z.id === "concourse-north") { z.currentDensity = 2.8; z.flowPerMinute = 32; }
      if (z.id === "concourse-south") { z.currentDensity = 3.9; z.flowPerMinute = 22; }
      if (z.id === "exit-east") { z.currentDensity = 1.5; z.flowPerMinute = 55; }
      return z;
    });
  } else if (scenario === "crowd_rush") {
    liveZones = liveZones.map(z => {
      if (z.id === "gate-a") { z.currentDensity = 5.2; z.flowPerMinute = 14; }
      if (z.id === "gate-c") { z.currentDensity = 4.9; z.flowPerMinute = 16; }
      if (z.id === "concourse-north") { z.currentDensity = 4.2; z.flowPerMinute = 20; }
      return z;
    });
  } else if (scenario === "gate_b_emergency") {
    liveZones = liveZones.map(z => {
      if (z.id === "gate-b") { z.currentDensity = 5.5; z.flowPerMinute = 5; }
      if (z.id === "concourse-north") { z.currentDensity = 4.5; z.flowPerMinute = 15; }
      return z;
    });
    ...
    liveZones = liveZones.map(z => {
      if (z.id === "exit-east") { z.currentDensity = 5.8; z.flowPerMinute = 12; }
      if (z.id === "concourse-south") { z.currentDensity = 4.6; z.flowPerMinute = 18; }
      return z;
    });
  }
...
const LOCALIZED_FALLBACKS: Record<string, {
  ...
  en: {
    langName: "English",
    initialGreeting: "Welcome back! Based on real-time sensor density, the optimal entryway for you is {name}. This route has {stepFree} accessibility.",
    fallbackTemplate: "System notice: StadiumPulse's AI reasoning layer is currently operating in local safety mode. Based on live turnstile counts, we strongly recommend utilizing {name}, which currently has {risk} density levels."
  },
  es: {
    langName: "Español",
    initialGreeting: "¡Bienvenido! Según la densidad de los sensores en tiempo real, la entrada óptima para usted es {name}. Esta ruta tiene accesibilidad {stepFree}.",
    fallbackTemplate: "Aviso del sistema: La capa de IA de StadiumPulse funciona en modo de seguridad local. Según el conteo físico de torniquetes, le recomendamos usar {name} con niveles de densidad {risk} en este momento."
  },
  fr: {
    langName: "Français",
    initialGreeting: "Bienvenue! Selon la densité des capteurs en temps réel, l'entrée optimale pour vous est {name}. Cet itinéraire a une accessibilité {stepFree}.",
    fallbackTemplate: "Avis du système: La couche d'IA de StadiumPulse fonctionne en mode de sécurité locale. Selon le comptage physique des tourniquets, nous vous recommandons d'utiliser {name} avec un niveau de densité {risk} actuellement."
  },
  de: {
    langName: "Deutsch",
    initialGreeting: "Willkommen! Basierend auf der Echtzeit-Sensordichte ist der optimale Eingang für Sie {name}. Diese Route ist {stepFree} zugänglich.",
    fallbackTemplate: "Systemhinweis: Die KI-Ebene von StadiumPulse arbeitet im lokalen Sicherheitsmodus. Basierend auf den physischen Zählungen empfehlen wir die Nutzung von {name} mit {risk} Dichte."
  },
  hi: {
    langName: "हिन्दी",
    initialGreeting: "स्वागत है! वास्तविक समय सेंसर घनत्व के आधार पर, आपके लिए सर्वोत्तम प्रवेश द्वार {name} है। इस मार्ग में {stepFree} पहुंच है।",
    fallbackTemplate: "सिस्टम सूचना: स्टेडियमपल्स की एआई परत स्थानीय सुरक्षा मोड में चल रही है। भौतिक गणना के आधार पर, हम {name} का उपयोग करने की सलाह देते हैं जहां अभी {risk} घनत्व है।"
  },
  ar: {
    langName: "العربية",
    initialGreeting: "مرحبًا بك! بناءً على كثافة المستشعرات في الوقت الفعلي، فإن المدخل الأمثل لك هو {name}. هذا المسار يتميز بالوصول {stepFree}.",
    fallbackTemplate: "تنبيه النظام: تعمل طبقة الذكاء الاصطناعي في وضع الأمان المحلي. بناءً على أعداد البوابات الفعلية، نوصي بشدة باستخدام {name} التي لديها مستويات كثافة {risk} الآن."
  },
  pt: {
    langName: "Português",
    initialGreeting: "Boas-vindas! Com base na densidade dos sensores em tempo real, a entrada ideal para você é {name}. Esta rota tem acessibilidade {stepFree}.",
    fallbackTemplate: "Aviso do sistema: A camada de IA do StadiumPulse está operando em modo de segurança local. Com base na contagem física de catracas, recomendamos utilizar {name} com nível de densidade {risk} agora."
  },
  ja: {
    langName: "日本語",
    initialGreeting: "ようこそ！リアルタイムのセンサー計測に基づき、最適な入り口は{name}です。このルートは{stepFree}に対応しています。",
    fallbackTemplate: "システム通知：StadiumPulseのAI推論レイヤーは現在ローカル安全モードで動作しています。リアルタイムの計測に基づき、現在混雑度が{risk}の{name}のご利用を強くお勧めします。"
  }
};
...
app.post("/api/assistant", async (req, res) => {
  ...
  if (!checkRateLimit(ip)) {
    res.status(429).json({
      ...
      details: "You have exceeded the rate limit. Please wait a few seconds before asking again."
    });
    return;
  }
  ...
  if (message.length > 500) {
    res.status(400).json({ error: "Message length exceeds the 500-character limit." });
    return;
  }

  // Optimize page loads and toggles: short-circuit if message is the automated initial recommendation query
  const isInitialRequest = message.includes("recommend the best entryway for a fan") || 
  ...
  if (isInitialRequest) {
    ...
    const localization = LOCALIZED_FALLBACKS[langCode] || LOCALIZED_FALLBACKS.en;
  ...
  try {
    ...
    const zonesContext = rankedZones.map(z => 
...
6. Ground your navigation recommendations strictly in the live sensor status provided above. Do not hallucinate raw numbers.`;
    ...
    const reply = response.text || "I apologize, but I could not compute a navigation path at this moment. Please follow the physical signage towards Gate A.";

    const finalRankedZones = attachDensityHistory(rankedZones);
    const finalRecZone = recommendedZone ? finalRankedZones.find(z => z.zoneId === recommendedZone.zoneId) || recommendedZone : null;
...
app.get("/api/operations", async (req, res) => {
  ...
  if (!checkOperationsRateLimit(ip)) {
    res.status(429).json({
      ...
      details: "You are polling too rapidly. Please slow down your polling frequency."
    });
    return;
  }
  try {
    ...
    if (currentScenario === "crowd_rush") {
      liveZones = liveZones.map(z => {
        ...
        if (z.id === "gate-a") {
          z.currentDensity = parseFloat(Math.max(4.6, Math.min(5.5, 5.2 + variance)).toFixed(2));
          z.flowPerMinute = Math.max(5, Math.min(30, 14 + flowVar));
        } else if (z.id === "gate-c") {
          z.currentDensity = parseFloat(Math.max(4.5, Math.min(5.4, 4.9 + variance)).toFixed(2));
          z.flowPerMinute = Math.max(5, Math.min(30, 16 + flowVar));
        } else if (z.id === "concourse-north") {
          z.currentDensity = parseFloat(Math.max(3.8, Math.min(4.8, 4.2 + variance)).toFixed(2));
      ...
      liveZones = liveZones.map(z => {
        ...
        if (z.id === "gate-b") {
          z.currentDensity = parseFloat(Math.max(5.1, Math.min(5.5, 5.4 + variance)).toFixed(2));
          z.flowPerMinute = Math.max(2, Math.min(8, 5 + flowVar));
        } else if (z.id === "concourse-north") {
          z.currentDensity = parseFloat(Math.max(4.0, Math.min(5.0, 4.5 + variance)).toFixed(2));
      ...
      liveZones = liveZones.map(z => {
        ...
        z.currentDensity = parseFloat(Math.max(0.5, Math.min(z.capacityPerSqm, z.currentDensity + 1.2 + variance)).toFixed(2));
        z.flowPerMinute = Math.max(5, Math.round(z.flowPerMinute * 0.4));
        return z;
      });
      ...
      liveZones = liveZones.map(z => {
        ...
        if (z.id === "exit-east") {
          z.currentDensity = parseFloat(Math.max(5.2, Math.min(6.0, 5.7 + variance)).toFixed(2));
          z.flowPerMinute = Math.max(5, Math.min(25, 12 + flowVar));
        } else if (z.id === "concourse-south") {
          z.currentDensity = parseFloat(Math.max(4.1, Math.min(5.2, 4.6 + variance)).toFixed(2));
    ...
    const breachingZones = finalZoneRisks.filter(z => z.riskLevel === "high" || z.flowStatus === "congested");
    ...
    if (breachingZones.length > 0) {
      const breachingZonesHash = breachingZones.map(z => `${z.zoneId}:${z.riskLevel}:${z.flowStatus}`).join("|");
      const now = Date.now();
      
      // Check if we are in the 5-minute quota lockout back-off period
      const inQuotaLockout = isAiQuotaExceeded && (now - aiQuotaExceededTime < 5 * 60 * 1000);
      ...
      if (cachedAiRecommendations && !inQuotaLockout) {
        const isIdentical = cachedAiRecommendations.breachingZonesHash === breachingZonesHash;
      ...
      if (inQuotaLockout) {
        console.warn("Gemini API is currently in back-off lockout. Using local rule-based safety fallbacks.");
        aiRecommendations = breachingZones.map(z => 
...
3. Be clear, calm, and directive. Avoid marketing or general feedback. Keep each recommendation under 15 words.`;
        ...
        try {
          ...
          console.error("AI operations recommendations failed, using local safety fallbacks:", err);
          ...
          if (errMsg.includes("429") || errMsg.includes("quota") || errMsg.includes("Quota") || errMsg.includes("RESOURCE_EXHAUSTED")) {
            ...
            console.warn("Quota exceeded (429) detected. Activating AI lockout back-off for 5 minutes.");
          }
      ...
      breachingZones.forEach((z, idx) => {
        if (z.riskLevel === "high") {
          const matchingRec = aiRecommendations[idx] || aiRecommendations[0] || "Hold boarding lines and shift crowd queues.";
          const alreadyLogged = activeIncidents.some(inc => inc.zoneId === z.zoneId && inc.status === "active" && inc.message.includes("high density"));
          if (!alreadyLogged) {
    ...
    res.json({
      ...
      isAiQuotaExceeded: isAiQuotaExceeded && (Date.now() - aiQuotaExceededTime < 5 * 60 * 1000)
    });

  } catch (error: any) {
    console.error("Operations endpoint crash:", error);
    res.status(500).json({
      error: "An unexpected error occurred while processing operations telemetry."
    });
  }
});
