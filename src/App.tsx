...
export default function App() {
  ...
  const [mapSelectedZoneId, setMapSelectedZoneId] = useState<string | null>(
    null,
  );
  ...
  // Announce newly arriving unread alerts to screen readers automatically
  useEffect(() => {
    const unread = notifications.filter((n) => !n.isRead);
    if (unread.length > 0) {
      // Announce the newest unread alert
      const latest = unread[0];
      setLiveAnnouncement(`New Operational Alert: ${latest.message}`);
    }
  }, [notifications]);
            ...
            opsData.zones.forEach((z: ZoneRisk) => {
              if (z.riskLevel === "high") {
                if (!seenHighRiskZones.current.has(z.zoneId)) {
                  ...
                  addNotification(
                    `CRITICAL ALERT: ${z.name} has entered high density congestion (${z.currentDensity} p/m²)!`,
                    "risk_alert",
                  );
                }
              } else {
        ...
        if (incRes.ok) {
          ...
          incData.forEach((inc: Incident) => {
            if (!seenIncidents.current.has(inc.id)) {
              ...
              addNotification(
                `NEW INCIDENT: ${inc.severity.toUpperCase()} - ${inc.message} in ${inc.zoneName}`,
                "incident",
              );
            }
          });
  ...
  const navItems = [
    {
      ...
      label: "Fan Assistant",
      icon: Users,
      desc: "Multilingual navigation chat",
    },
    {
      id: "dashboard" as ViewType,
      label: "Gold Command Dashboard",
      icon: Terminal,
      desc: "Stadium telemetry & details",
    },
    {
      id: "map" as ViewType,
      label: "Live Stadium Map",
      icon: Map,
      desc: "Interactive spatial grid viewer",
    },
    {
      id: "incidents" as ViewType,
      label: "Incident Log",
      icon: AlertOctagon,
      desc: "Emergency logistics tracker",
    },
    {
      id: "settings" as ViewType,
      label: "Settings/Accessibility",
      icon: Sliders,
      desc: "Accessibility & layout defaults",
    },
  ];
  ...
  const getViewTitle = () => {
    switch (activeTab) {
      case "fan":
        return "Multilingual Fan Co-Pilot";
      case "dashboard":
        return "Gold Command Control Centre";
      case "map":
        return "Live Stadium Spatial View";
      case "incidents":
        return "Active Emergency Logistics Audit";
      case "settings":
        return "Accessibility & Application Settings";
      default:
        return "StadiumPulse AI";
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex flex-col flex-1 min-h-0">
          ...
          >
            <div className="flex items-center gap-2.5">
              ...
              >
                <Compass
                  className={`h-5 w-5 ${reducedMotion ? "" : "animate-spin-slow"}`}
                />
              </div>
              {!isSidebarCollapsed && (
                <div>
                  <h1 className="text-sm font-black tracking-wider uppercase font-display flex items-center gap-1">
                    StadiumPulse{" "}
                    <span className="text-teal-600 font-normal">AI</span>
                  </h1>
            ...
            <button
              ...
              aria-label={
                isSidebarCollapsed
                  ? "Expand sidebar navigation text"
                  : "Collapse sidebar navigation text"
              }
            >
              {isSidebarCollapsed ? (
                ...
                >
                  ...
                  {!isSidebarCollapsed && (
                    <div className="min-w-0">
                      ...
                      <span
                        className={`text-[9px] block opacity-70 truncate mt-0.5 ${isActive ? "text-white/80" : ""}`}
                      >
                        {item.desc}
                      </span>
                    </div>
        ...
        {!isSidebarCollapsed ? (
          <div
            className={`p-4 border-t text-[10.5px] ${
              isHighContrast
                ? "border-white"
                : "border-neutral-100 bg-neutral-50/40 text-neutral-500"
            }`}
          >
            <div className="space-y-1.5">
              ...
              <span className="text-[10px] text-neutral-400 block leading-tight">
                Persistent WCAG contrast overrides.
              </span>
            </div>
            <button
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top Header Bar */}
        <header
          className={`border-b transition-all sticky top-0 z-20 ${
            isHighContrast
              ? "bg-black border-white text-white"
          ...
          <div className="px-4 py-3 flex items-center justify-between gap-4">
              ...
              <div>
                ...
                <div className="hidden sm:flex items-center gap-2 text-[10px] text-neutral-400 font-sans mt-0.5">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span>
                      Sensors polled:{" "}
                      <strong className="font-mono">{lastSensorUpdate}</strong>
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="relative">
                ...
                {showNotifications && (
                  ...
                  >
                    <div className="p-3 border-b border-neutral-100 flex items-center justify-between bg-neutral-50">
                      <span className="font-bold text-[10px] uppercase tracking-wider text-neutral-800">
                        Operational Alerts
                      </span>
                      <button
                        onClick={() => {
                        ...
                        notifications.map((n) => (
                          ...
                          >
                            <div className="flex justify-between items-start gap-1">
                              <span
                                className={`inline-block text-[8px] px-1.5 py-0.5 rounded font-mono uppercase shrink-0 ${
                                  n.type === "risk_alert"
                            ...
                            <p className="mt-1.5 text-[11px] leading-relaxed text-neutral-900">
                              {n.message}
                            </p>
                          </div>
                        ))
                ...
                {/* Live screen-reader announcer for incoming security and risk alerts */}
              ...
              >
                ...
                <div className="text-left hidden sm:block">
                  ...
                  <span className="text-[9px] font-mono opacity-65 uppercase">
                    {profile.defaultRole} mode
                  </span>
                </div>
              </button>
            </div>
              ...
              <div className="flex gap-4 items-start pr-8">
                ...
                <div className="space-y-1.5 text-xs">
                  ...
                  <p className="opacity-85 leading-relaxed text-[11px]">
                    StadiumPulse uses actual rule-based safety equations for
                    density risk metrics, and synthesizes that context in a{" "}
                    <strong>Gemini-2.5-Flash</strong> conversational model to
                    guide fans (Fan Assistant) and assist command operators
                    (Gold Command Console) symmetrically.
                  </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-2 text-[10px] text-teal-200">
                    <div>
                      ...
                      Calculates congestion and risk limits deterministically
                      for reliable safety logic.
                    </div>
                    <div>
                      <strong className="text-white block">
                        🌍 Intelligent Language
                      </strong>
                      Seamlessly auto-detects user languages or respects manual
                      preferred overrides.
                    </div>
                    <div>
                      <strong className="text-white block">
                        ♿ Highly Accessible Layout
                      </strong>
                      Features WCAG AAA contrast theme, reduced-motion bypasses,
                      and auto-read screen optimizations.
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-neutral-150 pb-4 mb-4">
                  <div>
                    ...
                    <p className="text-xs text-neutral-500 mt-0.5">
                      Visualizing critical stadium flows and security
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-mono text-neutral-400">
                      High-Contrast:{" "}
                      <strong>{isHighContrast ? "Active" : "Standard"}</strong>
                    </span>
                  </div>
                </div>
            ...
            {activeTab === "settings" && (
              ...
              >
                ...
                <p className="text-xs text-neutral-500 mb-6">
                  Manage persistent local language, screen display modes, and
                  automatic audio narration controls.
                </p>
                  ...
                  <div className="p-4 bg-neutral-50 rounded-xl border border-neutral-100 space-y-4 text-xs">
                    <div className="space-y-1">
                      ...
                      <span className="text-neutral-500 block text-[11px]">
                        Enables bright WCAG 2.1 AAA high contrast lines.
                      </span>
                      <button
                        onClick={toggleHighContrast}
                    ...
                    <div className="space-y-1">
                      ...
                      <span className="text-neutral-500 block text-[11px]">
                        Excludes panning waves and fast transition spins.
                      </span>
                      <button
                        onClick={() => {
                          saveProfile({
                    ...
                    <div className="space-y-1">
                      ...
                      <span className="text-neutral-500 block text-[11px]">
                        Narrate AI messages aloud immediately upon retrieval.
                      </span>
                      <button
                        onClick={() => {
                          saveProfile({
        ...
        <footer
          className={`border-t py-5 mt-12 text-center text-xs transition-all ${
            isHighContrast
              ? "bg-black border-white text-white"
        ...
        >
          <div className="px-4 space-y-1.5">
            <p>
              © 2026 StadiumPulse AI — Smart Stadium & Tournament Operations
              (FIFA Challenge 4)
            </p>
            <p className="text-[10px] opacity-70">
              Unified rule-based density matrix scoring grounded on real-time
              sensory feeds.
            </p>
          </div>
        </footer>
      ...
      {isProfileOpen && (
        ...
        >
          ...
          <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
            <div
              className={`w-screen max-w-sm border-l transition-all ${
                isHighContrast
                  ? "bg-black border-white text-white"
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between border-b pb-4">
                    <h3
                      ...
                      className="font-display font-black text-sm uppercase tracking-wider flex items-center gap-2"
                    >
                  ...
                  <div className="p-3 bg-teal-50/50 border border-teal-100 rounded-xl text-[10.5px] leading-relaxed text-teal-800">
                    <strong>Local Environment Storage Only:</strong> All display
                    modifications are kept on this physical client browser index
                    for safety (No login accounts, no PII tracking).
                  </div>
                  ...
                  <div className="space-y-4 text-xs">
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        <option value="auto">
                          Auto-Detect Query Language (Default)
                        </option>
                      ...
                      <span className="text-[10px] text-neutral-400 leading-tight block">
                        Saves manual choice override for Gemini conversation
                        returns.
                      </span>
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <span className="text-[10px] text-neutral-400 leading-tight block">
                        Determines default landing screen upon application
                        reload.
                      </span>
                    </div>
                    ...
                    <div className="space-y-2.5 pt-2 border-t">
                      ...
                      <label className="flex items-center justify-between cursor-pointer select-none">
                        ...
                        />
                        <div
                          className={`w-9 h-5 rounded-full transition-colors relative ${editContrast ? "bg-teal-600" : "bg-neutral-300"}`}
                        >
                          <div
                            className={`w-3.5 h-3.5 rounded-full bg-white absolute top-0.75 transition-transform ${editContrast ? "translate-x-4.5" : "translate-x-0.75"}`}
                          />
                        </div>
                      </label>
                      ...
                      <label className="flex items-center justify-between cursor-pointer select-none">
                        ...
                        />
                        <div
                          className={`w-9 h-5 rounded-full transition-colors relative ${editMotion ? "bg-teal-600" : "bg-neutral-300"}`}
                        >
                          <div
                            className={`w-3.5 h-3.5 rounded-full bg-white absolute top-0.75 transition-transform ${editMotion ? "translate-x-4.5" : "translate-x-0.75"}`}
                          />
                        </div>
                      </label>
                      ...
                      <label className="flex items-center justify-between cursor-pointer select-none">
                        ...
                        />
                        <div
                          className={`w-9 h-5 rounded-full transition-colors relative ${editTTS ? "bg-teal-600" : "bg-neutral-300"}`}
                        >
                          <div
                            className={`w-3.5 h-3.5 rounded-full bg-white absolute top-0.75 transition-transform ${editTTS ? "translate-x-4.5" : "translate-x-0.75"}`}
                          />
                        </div>
                      </label>
                    </div>
