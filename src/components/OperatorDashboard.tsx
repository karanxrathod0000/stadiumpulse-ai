...
export default function OperatorDashboard({ isHighContrast, reducedMotion, userProfile }: OperatorDashboardProps) {
  ...
  const averageDensity = totalZones > 0
    ? parseFloat((zones.reduce((sum, z) => sum + z.currentDensity, 0) / totalZones).toFixed(2))
    : 0;
  ...
  const getRiskColor = (level: string) => {
    if (isHighContrast) {
      switch (level) {
        case "high": return "bg-black text-yellow-400 border-4 border-yellow-400 font-bold";
        case "medium": return "bg-black text-blue-400 border-4 border-blue-400 font-bold";
        default: return "bg-black text-white border-4 border-white font-bold";
      }
    }
    switch (level) {
  ...
  return (
    <div className="space-y-6" id="operator-dashboard-container">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4" id="telemetry-stats-bar">
        ...
        <div className="p-4 rounded-xl border border-neutral-200 bg-white flex items-center gap-3.5 shadow-xs">
          ...
          <div>
            <span className="text-neutral-500 block text-[10px] font-bold uppercase tracking-wider">Average Density</span>
            <span className="text-xl font-bold font-mono text-neutral-900">{averageDensity} <span className="text-xs font-normal text-neutral-400">p/m²</span></span>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl border border-neutral-200 bg-white flex items-center gap-3.5 shadow-xs">
          ...
          <div>
            <span className="text-neutral-500 block text-[10px] font-bold uppercase tracking-wider">High Risk Zones</span>
            <span className="text-xl font-bold font-mono text-rose-600">{highRiskZones} <span className="text-xs font-normal text-neutral-400">/ {totalZones}</span></span>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl border border-neutral-200 bg-white flex items-center gap-4 shadow-xs">
          ...
          <div>
            <span className="text-neutral-500 block text-[10px] font-bold uppercase tracking-wider">Active Incidents</span>
            <span className="text-xl font-bold font-mono text-amber-600">{incidents.filter(i => i.status === "active").length}</span>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl border border-neutral-200 bg-white flex flex-col justify-center space-y-1 shadow-xs">
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs text-neutral-400 font-mono">Refreshed: {lastRefreshed || "loading"}</span>
            <button
      ...
      <div className={`p-5 rounded-2xl border transition-all ${
        isHighContrast ? "bg-black border-4 border-white text-white" : "bg-gradient-to-r from-teal-50/50 to-neutral-50/50 border-neutral-200/80 shadow-xs"
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-neutral-200/60 pb-3 mb-4">
          <div>
            <h4 className="font-display font-bold text-xs uppercase tracking-wider text-neutral-900 flex items-center gap-2">
              <span className="p-1 bg-teal-100 text-teal-700 rounded-md shrink-0">⚡</span>
              FIFA World Cup 2026 Telemetry Scenario Simulator
            </h4>
            <p className="text-[11px] text-neutral-500">
              For evaluation: Inject real-time crowd dynamics, crises, or weather alerts to verify immediate AI and rule-engine adaptation.
            </p>
          </div>
          <div className="shrink-0">
            <span className={`text-[10px] font-extrabold px-3 py-1 rounded-full uppercase tracking-wider border flex items-center gap-1.5 ${
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          ...
          >
            ...
            <span className="text-[10px] text-neutral-400 block leading-tight mt-1 font-normal">Normal baseline crowd flow and fluctuations.</span>
          </button>
          ...
          >
            <span className="text-xs font-bold block text-amber-700">🔥 Crowd Rush</span>
            <span className="text-[10px] text-neutral-400 block leading-tight mt-1 font-normal">Spike Gate A & C northwest/south entries.</span>
          </button>
          ...
          >
            <span className="text-xs font-bold block text-rose-700">⚠️ Gate B Lock</span>
            <span className="text-[10px] text-neutral-400 block leading-tight mt-1 font-normal">Simulate scanning node outage at Gate B.</span>
          </button>
          ...
          >
            <span className="text-xs font-bold block text-blue-700">🌧️ Heavy Rain</span>
            <span className="text-[10px] text-neutral-400 block leading-tight mt-1 font-normal">Universal pedestrian slow footing velocity.</span>
          </button>
          ...
          >
            <span className="text-xs font-bold block text-purple-700">🚪 Egress Surge</span>
            <span className="text-[10px] text-neutral-400 block leading-tight mt-1 font-normal">Simulate transit outbound links post-match egress.</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-neutral-200/80 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-neutral-100 pb-3">
              <div>
                ...
                <p className="text-xs text-neutral-500">
                  Interactive safety grid mapping densities, flow velocities, and hazard thresholds.
                </p>
              </div>
            ...
            {zones.length === 0 ? (
              <div className="py-20 text-center text-xs text-neutral-400 space-y-2">
                ...
                <p>Establishing secure connection with stadium node routers...</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4" id="zones-grid-deck">
                {zones.map((zone) => {
                  const percentOfCapacity = Math.min(100, Math.round((zone.currentDensity / zone.capacityPerSqm) * 100));
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        <div className="flex items-start justify-between gap-1.5">
                          <span className="font-bold font-display text-xs text-neutral-950 truncate max-w-[50%]" title={zone.name}>
                            {zone.name}
                          </span>
                          <div className="flex items-center gap-2 shrink-0">
                            <Sparkline history={zone.densityHistory} isHighContrast={isHighContrast} width={60} height={20} />
                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${getRiskColor(zone.riskLevel)}`}>
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-neutral-400 uppercase font-bold tracking-wider">
                          <span>{zone.type}</span>
                          <span>•</span>
                          <span>{zone.stepFreeAccess ? "Step-Free" : "Stairs Only"}</span>
                        </div>
                      </div>
                      ...
                      <div className="my-3.5 space-y-1">
                        <div className="flex justify-between items-center text-[10px] font-mono text-neutral-500">
                          ...
                          <span className="font-bold text-neutral-800">{percentOfCapacity}% capacity</span>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs pt-2.5 border-t border-neutral-100 font-mono">
                        <div>
                          <span className="text-neutral-400 block text-[9px] font-bold uppercase">Density</span>
                          <span className="font-bold text-neutral-800">{zone.currentDensity} <span className="text-[10px] font-normal">p/m²</span></span>
                        </div>
                        <div>
                          <span className="text-neutral-400 block text-[9px] font-bold uppercase">Velocity Flow</span>
                          <span className={`font-bold ${
                            zone.flowStatus === "congested" ? "text-rose-600" : "text-neutral-800"
                          }`}>
                            {zone.flowPerMinute} <span className="text-[10px] font-normal">p/min/m</span>
                          </span>
                        </div>
                      </div>
                      ...
                      {zone.recommendedAction && (
                        <div className="mt-3 p-2 bg-neutral-50/70 border border-neutral-200/50 rounded-lg text-[10px] text-neutral-600">
                          <span className="font-bold text-neutral-800 block mb-0.5">Rule-Engine Trigger:</span>
                          {zone.recommendedAction}
                        </div>
                      )}
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-white p-5 rounded-2xl border border-neutral-200/80 shadow-sm space-y-4">
            ...
            {isAiQuotaExceeded && (
              <div className="p-3 bg-rose-50 border border-rose-200 text-rose-900 rounded-xl text-xs space-y-1" id="ai-quota-alert-banner">
                <div className="flex items-center gap-1.5 font-bold text-rose-800">
                  <span className="h-2 w-2 rounded-full bg-rose-500 animate-pulse shrink-0" />
                  Free-tier AI Quota Reached (429 Limit)
                </div>
                <p className="text-[10.5px] text-rose-700 leading-normal">
                  The daily free request quota for gemini-3.5-flash has been hit. StadiumPulse has automatically activated local rule-based safety algorithms to ensure continuous operational integrity.
                </p>
              </div>
            )}
            ...
            {aiRecommendations.length === 0 ? (
              <div className="py-8 text-center text-xs text-neutral-400 bg-neutral-50/50 rounded-xl border border-neutral-200/40 p-4 space-y-2">
                ...
                <p className="font-bold text-neutral-900">No active recommendations — all zones nominal</p>
                <p className="text-[11px] leading-relaxed">
                  No gates or concourses are currently breaching safe limits (Density &lt; 4.5 & Flow &gt; 25). The AI reasoning invocation was skipped to optimize system throughput.
                </p>
              </div>
            ) : (
              <div className="space-y-2.5">
                <div className="p-3 bg-neutral-50 border border-neutral-200 rounded-xl text-xs flex gap-2">
                  ...
                  <p className="text-neutral-500 text-[11px]">
                    The LLM has compiled high-density and congested areas into these direct operational interventions.
                  </p>
                </div>
                ...
                {aiRecommendations.map((rec, idx) => (
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-neutral-700 leading-relaxed font-semibold">{rec}</p>
                    </div>
                    <button
          ...
          <div className="bg-white p-5 rounded-2xl border border-neutral-200/80 shadow-sm space-y-4">
            ...
            {showAddForm && (
              <form onSubmit={handleCreateIncident} className="p-4 bg-neutral-50 rounded-xl border border-neutral-200 text-xs space-y-3">
                <div className="flex justify-between items-center border-b border-neutral-100 pb-1.5 mb-1">
                  <span className="font-bold text-neutral-900">Create Manual Emergency Dispatch</span>
                  <button 
                    type="button" 
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-[10px] text-neutral-400 uppercase block mb-1">Select Area</label>
                    <select 
                  ...
                  <div>
                    <label className="text-[10px] text-neutral-400 uppercase block mb-1">Severity</label>
                    <select 
                      className="w-full p-2 bg-white rounded border border-neutral-300 focus:outline-hidden"
                      value={formSeverity}
                ...
                <div>
                  <label className="text-[10px] text-neutral-400 uppercase block mb-1">Alert Message</label>
                  <textarea
                ...
                {incidents.map((inc) => (
                  ...
                  >
                    ...
                    <div className="space-y-1 flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <span className={`font-bold font-display ${inc.status === "resolved" ? "line-through" : ""}`}>
                          {inc.zoneName}
                        </span>
                      ...
                      <p className={`text-[11px] leading-relaxed ${inc.status === "resolved" ? "line-through opacity-80" : ""}`}>
                        {inc.message}
                      </p>
