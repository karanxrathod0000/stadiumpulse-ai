...
export default function StadiumMap({
  ...
  const getZoneCoords = (id: string) => {
    switch (id) {
      case "gate-a":
        return { x: 160, y: 110 }; // NW
      case "gate-b":
        return { x: 440, y: 110 }; // NE
      case "gate-c":
        return { x: 300, y: 310 }; // South
      case "concourse-north":
        return { x: 300, y: 120 }; // North Concourse
      case "concourse-south":
        return { x: 300, y: 280 }; // South Concourse
      case "exit-east":
        return { x: 520, y: 200 }; // Transit Exit East
      default:
        return { x: 300, y: 200 };
    }
  };
  ...
  const activeZone = zones.find(
    (z) => z.zoneId === (selectedZoneId || hoveredZoneId),
  );
  ...
  return (
    ...
    >
      ...
      <div className="relative bg-neutral-900 rounded-xl overflow-hidden p-2 border border-neutral-850 h-[280px] flex items-center justify-center">
        ...
        >
          ...
          >
            ...
            <path
              d="M 160 110 Q 230 110 300 120"
              stroke={highlightStepFree ? "#3b82f6" : undefined}
              strokeWidth={highlightStepFree ? 3.5 : undefined}
            />
            {/* Gate C to South Concourse */}
            <path
              d="M 300 310 Q 300 295 300 280"
              stroke={highlightStepFree ? "#3b82f6" : undefined}
              strokeWidth={highlightStepFree ? 3.5 : undefined}
            />
          ...
          {(() => {
            ...
            if (!z) return null;
            const active =
              selectedZoneId === "concourse-north" ||
              hoveredZoneId === "concourse-north";
          ...
          {(() => {
            ...
            if (!z) return null;
            const active =
              selectedZoneId === "concourse-south" ||
              hoveredZoneId === "concourse-south";
            const color = getRiskColorHex(z.riskLevel, active);
            return (
              <path
            ...
            return (
              ...
              >
                ...
                <circle
                  ...
                  className={
                    zone.riskLevel === "high" && !reducedMotion
                      ? "animate-ping"
                      : ""
                  }
                />
                ...
                {isGate ? (
                  ...
                  <polygon
                    points={
                      active ? "0,-10 10,0 0,10 -10,0" : "0,-8 8,0 0,8 -8,0"
                    }
                    fill={color}
                    stroke="#171717"
                    strokeWidth="2.5"
                ...
                >
                  {zone.zoneId === "exit-east"
                    ? "TRANSIT"
                    : zone.zoneId.replace("gate-", "GATE ").toUpperCase()}
                </text>
              </g>
            );
          })}
      ...
      <div
        className={`mt-3 p-3.5 rounded-xl border flex flex-col justify-between min-h-[75px] h-auto transition-all ${
          isHighContrast
      ...
      >
        {activeZone ? (
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 h-full">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px] text-neutral-500 font-mono mt-0.5 truncate">
                Density: {activeZone.currentDensity} p/m² | Flow:{" "}
                {activeZone.flowPerMinute} p/min/m
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 shrink-0">
              <div className="text-right">
                <span
                  className={`inline-block text-[9px] font-bold px-2 py-0.5 rounded-full uppercase border ${
          ...
          <div className="flex items-center justify-center gap-1.5 h-full text-center text-neutral-400 text-[11px] italic">
            <Compass
              className={`h-4 w-4 text-neutral-300 ${reducedMotion ? "" : "animate-spin-slow"}`}
            />
            <span>
              Click or hover any map zone above to inspect live telemetry
            </span>
          </div>
        )}
      </div>
